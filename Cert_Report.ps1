...
$Output = Get-CA | Get-IssuedRequest -Filter "NotAfter -ge $(Get-Date)", "NotAfter -le $((Get-Date).AddMonths(4))" | select RequestID, CommonName, NotAfter, CertificateTemplate | sort Notafter
